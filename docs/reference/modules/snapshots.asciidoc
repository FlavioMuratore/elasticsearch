...
Note:  If using the AWS Managed ElasticSeach service, you need to register the snapshot repository following the steps here: http://docs.aws.amazon.com/elasticsearch-service/latest/developerguide/es-managedomains.html#es-managedomains-snapshot-registerdirectory
...
`readonly`:: Makes repository read-only. added[2.1.0]  Defaults to `false`.
